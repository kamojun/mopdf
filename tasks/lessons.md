...
### セレクタなしのQSSは子のQMenuにも波及する

- **状況**: ページ上の右クリックメニューで、ハイライト中の項目が白地に白文字になって読めなかった
- **原因**: `PageWidget.setStyleSheet("background: white; ...")` はセレクタなしなので、`QMenu(self)` で作った子メニューにも適用されていた
- **ルール**: `setStyleSheet` を持つウィジェットの子としてポップアップ（`QMenu` など）を作らない。作る場合はQSSを `ClassName { ... }` のようにセレクタで限定する

### ファイル編集はEditツールで行う（python/sedで書き換えない）

- **状況**: 複数箇所の置換をまとめるためにBashからpythonスクリプトで`pdf_viewer.py`を書き換えようとして拒否された
- **ルール**: ファイル編集は必ずEdit/Writeツールで行う。箇所が多くてもEditを複数回呼ぶ。python/sedでの一括置換はしない（変更内容がレビューしづらいため）
