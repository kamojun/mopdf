...
## TOC項目クリックによる意図しないページジャンプの防止（計画中）
...
**採用する方針**: シングルクリックの瞬間には即ジャンプさせず、システムのダブルクリック判定時間（`QApplication.doubleClickInterval()`）だけ`QTimer`で遅延させる。その間に同じ項目への2回目のクリック（ダブルクリック=編集開始）が来たら、保留中のジャンプをキャンセルする。

- [ ] `TocPanel`に保留中ジャンプの状態（`item`, `column`）を保持する変数を追加
- [ ] `_on_item_clicked`: 即時 `emit` せず、`QTimer.singleShot(QApplication.doubleClickInterval(), ...)` で遅延実行するよう変更
- [ ] `_on_item_double_clicked`: 呼ばれたら該当項目の保留中ジャンプをキャンセルしてから既存の編集開始処理を行う
- [ ] `_toggle_page_edit_mode`（「#」連続入力モード開始時の`editItem`呼び出し）でも、対象項目の保留中ジャンプをキャンセルする（選択→即#ボタンで起点にするフローに対応）
- [ ] 保留中に選択が別項目に移った場合（`itemSelectionChanged`）も保留ジャンプを破棄する（stale jump防止）
- [ ] 動作確認: (1)単発クリック→遅延後に正しくジャンプする (2)ダブルクリックで編集開始→ジャンプが起きない (3)クリック直後に選択が変わる/連続入力モードの起点にする→ジャンプが起きない、の3パターンを実GUIで確認
